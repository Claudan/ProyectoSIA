...
namespace FID3Lib {
	...
	std::vector<std::string> FID3::getClases()
	{
		return clases;
	}
	std::vector<Elemento> FID3::getElementos() {
		return elementos;
	}
	...
	double FID3::gananciaAtributo(std::string _nombreAtributo, std::map<std::string, double>  _tipos, std::vector<Elemento> vecELementos)
	{
		...
		for (std::map<std::string, double>::const_iterator it = _tipos.begin(); it != _tipos.end(); it++)
		{
			...
			std::cout << "HAtributo " << _nombreAtributo << " : " << temp[0] << std::endl;
			HTotal += HAtributo * (entropiaConjuntoAtributo / entropiaConjuntoTotal);
		}
		std::cout << "HTotal de " << _nombreAtributo << " : " << HTotal << std::endl;
		std::cout << "entropia de clases " << _nombreAtributo << " : " << entropiaDeClases(vecELementos)<< std::endl;
	...
	double* FID3::entropiaAtributoDifuso(std::string nombreAtributo, std::string nombreDifuso, std::vector<Elemento> vecELementos)
	{
		...
		//std::cout << entropias[0] << ":" << entropias[1] << std::endl;
		//
		return entropias;
	}
	...
	void FID3::insertaHijosFID3(Nodo* nodo, std::vector<AtributoDifuso> _atributosDifusos, AtributoDifuso atribDifSeleccionado, std::vector<Elemento> vecELementos)
	{
		...
		for (std::map<std::string, double>::const_iterator it = _tiposADif.begin(); it != _tiposADif.end(); it++)
		{
			...
			std::cout << "proporcion de clase Y " << nombreAtributo << "->" << nombreDifuso <<": " <<proporcionClase["Y"]<<std::endl;
			std::cout << "proporcion de clase N " << nombreAtributo << "->" << nombreDifuso <<": " <<proporcionClase["N"] << std::endl;
			...
			if ( valorMayor>thetaR || atributosDifusos.size() == 0) {// || proporcionAtributoEnDataset(nombreAtributo)< thetaN
				...
				Nodo *nodoS = nuevoNodo(claseMayor, nombreAtributo, nombreDifuso,true, proporcionClase); // Lo modifique, incorporo ramaNombrePadre
				...
				//--------------------------------------------------------
				std::cout << "-----------------------------\n";
				std::cout << "nombreClase: " << claseMayor <<": "<<valorMayor << std::endl;
				std::cout << "-----------------------------\n";
				//atributosDifusos
				...
				for (unsigned int i = 0; i < atributosDifusosR.size(); ++i) {
					...
					//std::cout << ganancia << atributosDifusosR[i].getNombre() << atributosDifusosR[i].getTipos()["baja"]  << std::endl;
				...
				Nodo* nodoS = nuevoNodo(atributoSeleccionado, nombreAtributo, nombreDifuso,false, proporcionClase);
				(nodo->hijo).insert((nodo->hijo).end(), nodoS);
	...
	void FID3::recorrePreOrden(Nodo* raiz, Elemento *elemento, std::stack<double> *ambiguedad, double min) {
		...
		if ((*raiz).esHoja) {
			...
			std::cout << "La clase clasificada es: " + (*raiz).nombreAtributo +" con ambiguedad de: "+ std::to_string(min) << std::endl;
		    
			if ((*elemento).getClases()[(*raiz).nombreAtributo] < min) {
				std::cout << " Valor antiguo de la clase: " + std::to_string((*elemento).getClases()[(*raiz).nombreAtributo]) << std::endl;
				// Se agrega el valor de minimo a esa clase, en el caso de que siempre fuera 0, el resultado no se escribe. Se debería cambiar si hay tiempo.
				(*elemento).setClases((*raiz).nombreAtributo, min);
				std::cout << " Clase: " + (*raiz).nombreAtributo << std::endl;
				std::cout << "Nuevo valor : " + std::to_string((*elemento).getClases()[(*raiz).nombreAtributo]) << std::endl;
			}
		...
		if (!(*raiz).esHoja) {
			...
			for (int i = 0; i < (*raiz).hijo.size(); i++) {
				std::cout << "El minimo:" << min << std::endl;
				std::cout << "Se elige el siguiente camino: " + (*raiz).nombreAtributoPadre + " : " +(*raiz).nombreRamaPadre << std::endl;
				(*ambiguedad).push((*elemento).getValorDifuso((*raiz).nombreAtributoPadre, (*raiz).nombreRamaPadre));
				std::cout << "El valor difuso de ese camino es:" + std::to_string((*ambiguedad).top()) << std::endl;
